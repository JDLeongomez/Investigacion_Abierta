...
```{r fig1, echo = FALSE, out.width = "400px", fig.align = "center", fig.cap = "\\textbf{Figura 1.} Parámetros necesarios para calcular el tamaño de muestra necesario, para obtener un poder estadístico deseado."}
...
## Tamaño del efecto en *jpower* para *jamovi*: $\delta$ de Cliff

*jpower* usa, como tamaño del efecto para pruebas-*t*, el $\delta$ (delta) de Cliff [@cliffOrdinalMethodsBehavioral1996]. Es un tamaño de efecto comparable al más común $d$ de Cohen, pero que tiene una ventaja importante: a diferencia del $d$ de Cohen, que asume de distribuciones normales, el $\delta$ de Cliff es un tamaño de efecto no paramétrico, que como tal no tiene supuestos respecto a la distribución de los datos. Esto es especialmente útil al planear un estudio, cuando no se conoce la distribución de los datos que se obtendrán, ni si los resultados serán analizados con pruebas paramétricas o no paramétricas.

Adicionalmente, el $\delta$ de Cliff es sumamente intuitivo: si, por ejemplo, tenemos dos grupos (Grupo 1 y Grupo 2), y los comparamos, el $\delta$ de Cliff es sencillamente la probabilidad de que un valor tomado aleatoriamente del Grupo 2, sea mayor a un valor tomado aleatoriamente del Grupo 1, menos la probabilidad contraria (que un valor tomado aleatoriamente del Grupo 1, sea mayor a un valor tomado aleatoriamente del Grupo 2).

Como indicación de la relación entre estas medidas de tamaño del efecto, en la **Tabla 3** comparo valores del ($\delta$) de Cliff con los del $d$ de Cohen, resaltando sin embargo las definiciones de referencia propuestas por Cohen, para comparación de valores equivalentes. Como lo he discutido ampliamente, es importante tener en cuenta siempre que pensar tamaños del efecto en términos de "Pequeños", "Medios", o "Grandes" tiene serias limitaciones y su uso indiscriminado ha sido seriamente cuestionado [ver e.g. @bakkerSmallMediumLarge2019; @correllAvoidCohenSmall2020], y la *importancia* de un tamaño de efecto cambia según la disciplina y el fenómeno estudiado (para una discusión de las limitaciones de las definiciones de Cohen, ver sección [1.3.1 Por qué no es buena idea usar las definiciones de Cohen](#probs)).

```{r tab3, echo = FALSE}
colnames <- c("Definición de Cohen \n (¡solo como referencia!)",
              "$d$ de Cohen",
              "Delta de Cliff ($\\delta$)")

tab3 <- data.frame(ref = c("", "Pequeño","", "", "Medio","", "", "Grande", rep("", times = 2)),
                    cohen = seq(0.1, 1.0, by = 0.1),
                    cliff = c(cohd2delta(0.1),
                              cohd2delta(0.2),
                              cohd2delta(0.3),
                              cohd2delta(0.4),
                              cohd2delta(0.5),
...
kable(tab3,
      ...
      col.names = linebreak(colnames,
                            align = "c"),
      caption = "\\textbf{Tabla 3.} Equivalencia de valores de referencia para $d$ de Cohen y delta de Cliff ($\\delta$)") %>%
  kable_styling(latex_options = c("HOLD_position")) %>%
  column_spec(1, bold = TRUE) %>%
  row_spec(c(2,5,8), background = "#c4c4c4") %>%
  footnote(general = "Transformación hecha con la función \\\\href{https://www.rdocumentation.org/packages/orddom/versions/3.1/topics/cohd2delta}{cohd2delta} del paquete \\\\href{https://www.rdocumentation.org/packages/orddom/versions/3.1}{orddom}.",
           threeparttable = TRUE,
           escape = FALSE,
           general_title = "Nota:")
```

En cualquier caso, existen calculadoras para el $\delta$ de Cliff disponibles, bien sea a partir de una base de datos, o para convertir un $d$ de Cohen en un $\delta$ de Cliff. Por ejemplo:

-   En *R*, hay opciones como:

    -   La función [`cliff.delta`](https://www.rdocumentation.org/packages/effsize/versions/0.8.0/topics/cliff.delta) del paquete [`effsize`](https://www.rdocumentation.org/packages/effsize/versions/0.8.0), que permite calcular el $\delta$ de Cliff a partir de una base de datos.
    -   La función [`cohd2delta`](https://www.rdocumentation.org/packages/orddom/versions/3.1/topics/cohd2delta) del paquete [`orddom`](https://www.rdocumentation.org/packages/orddom/versions/3.1) que permite calcular un $\delta$ de Cliff a partir de un $d$ de Cohen [la comparación de se basa en el método descrito en @hubertyGroupOverlapBasis2000].\

-   Si prefieres no usar *R*:

    -   [*Cliff's Delta Calculator (CDC)*](https://iipus.webs.com/), es una pequeña aplicación gratuita [ver @macbethCliffDeltaCalculator2011, que contiene instrucciones para su descarga y uso], e incluso macros de *Excel* [ver @goedhartCalculationDistributionFree2016]. Permite calcular el $\delta$ de Cliff a partir de una base de datos.
    -   En [esta página](https://www.real-statistics.com/non-parametric-tests/mann-whitney-test/cliffs-delta/), Charles Zaiontz explica cómo calcular el $\delta$ de Cliff en *Excel*, a partir de una base de datos.
...
Primero, *R* es un software completamente **libre** y **gratuito** y, al ser abierto, sus avances y funciones no están limitados por lo que una compañía implemente (típicamente con fines comerciales y sujeto a las leyes de oferta y demanda), sino que depende directamente del trabajo colaborativo, y sin ánimo de lucro, de millones de personas alrededor del mundo; por esto, avanza más rápido que cualquier *software* comercial, y siempre tiene opciones para poder implementar las técnicas más novedosas[^9]. Segundo, al ser en últimas un lenguaje de programación, permite hacer prácticamente cualquier cosa que se pueda hacer en un computador[^10], explotando realmente sus capacidades.

[^9]: Prácticamente cada *nerd* que desarrolla una nueva prueba o técnica de análisis de datos, hace, además de una publicación, un paquete para *R*.

[^10]: Solo como un ejemplo, este documento en PDF, [mi sitio web](https://jdleongomez.info/es/), y [mi hoja de vida](https://jdleongomez.info/es/files/jdl_cv_es.pdf), fueron todos creados en *R*.
...
En las siguientes secciones, el código de [*R*](https://www.r-project.org/) está siempre resaltado sobre un fondo oscuro. Para correr estos códigos, puedes sencillamente copiarlos y pegarlos en un *script*[^11], o directamente en la consola de [*R*](https://www.r-project.org/).

[^11]: [Este video](https://youtu.be/ejQ0BS2gVJI), hecho por [Juan Carlos Correa](https://docentes.konradlorenz.edu.co/2019/06/juan-carlos-correa.html) [-@correaScriptsVideo2020], es un ejemplo de introducción al uso de *scripts* de *R* pero por supuesto hay muchos disponibles.
...
Finalmente, puedo ver la asociación entre el tamaño de la muestra y el poder estadístico gráficamente con la función `plot`[^14], introduciendo como argumento mi objeto (en este caso `pcorr`) puedo ver una figura de este análisis (**Figura 2**).

[^14]: Las figuras que produce la función `plot` para análisis hechos en el paquete `pwr`, son objetos de clase `ggplot`, por lo que alguien familiarizado con el paquete [`ggplot2`](https://ggplot2.tidyverse.org/) puede modificar la figura para que, por ejemplo, los ejes, título y anotaciones estén en español, o para cambiar el tema y colores.

```{r fig2, fig.cap="**Figura 2.** Asociación entre el tamaño de la muestra y el poder estadístico para una correlación de 0.3, con un $\\alpha$ de 0.05, producida con la función `plot` del paquete `pwr`. Como se puede ver, sugiere un *n* de 112 (línea azul), para alcanzar un poder de 0.9 (90%)."}
plot(pcorr)
```
...
```{r fig3, fig.cap="**Figura 3.** Asociación entre el tamaño de la muestra y el poder estadístico para una correlación **positiva** (de una cola), de **al menos** 0.3, con un $\\alpha$ de 0.05, producida con la función `plot` del paquete `pwr`. Como se puede ver, sugiere un *n* de 67 (línea azul), para alcanzar un poder de 0.9 (90%)."}
plot(pcorrgreater)
```
...
```{r fig4, fig.height = 3.4, fig.cap="**Figura 4.** Asociación entre el tamaño de la muestra y el poder estadístico para una prueba-*t* de 0.4, con un $\\alpha$ de 0.05, producida con la función `plot` del paquete `pwr`. Como se puede ver, sugiere un *n* de 133 por grupo (línea azul), para alcanzar un poder de 0.9 (90%)."}
plot(ptInd)
```
...
```{r fig5, fig.cap="**Figura 5.** Asociación entre el tamaño de la muestra y el poder estadístico para una prueba-*t* de **al menos** 0.4 (una cola), con un $\\alpha$ de 0.05, producida con la función `plot` del paquete `pwr`. Como se puede ver, sugiere un *n* de 55 (línea azul), para alcanzar un poder de 0.9 (90%)."}
plot(ptRepGreat)
```

## Análisis de poder para ANOVAs de una vía. Función `pwr.anova.test`

Por lo menos hasta ahora[^15], `pwr` solo permite hacer análisis de poder para ANOVAs de una vía, balanceados (mismo número de participantes u observaciones por grupo), y de medidas independientes (es decir, comparando un número *k* de grupos)[^16].

[^15]: La versión actual de `pwr` es la 1.3-0.

[^16]: Para diseños más complejos, incluyendo diseños factoriales de medidas independientes, repetidas, o mixtas (tanto factores de medidas independientes como repetidas), la mejor opción actualmente es el paquete `Superpower`, descrito en la siguiente sección.
...
```{r fig6, fig.cap="**Figura 6.** Asociación entre el tamaño de la muestra y el poder estadístico para un ANOVA de 0.4, con un $\\alpha$ de 0.05, producida con la función `plot` del paquete `pwr`. Como se puede ver, sugiere un *n* de 58 participantes por grupo (línea azul), para alcanzar un poder de 0.9 (90%)."}
plot(panova)
```
...
3.  Interacción entre género y nivel educativo, que me dice si el salario depende simultáneamente del género y el nivel educativo. Por ejemplo, si entre las mujeres ganan más dinero las personas con postgrado, pero entre los hombres ganan más las personas con pregrado[^17].

[^17]: Obviamente este ejemplo muy seguramente no representa la realidad. Es solamente un ejemplo hipotético.
...
Este proceso es sumamente complejo, y no se puede solucionar de manera matemática *sencilla*[^18], por lo que `Superpower` usa una estrategia interesante: simula una base de datos para el diseño propuesto (dadas una serie de características de cada variable y su relación con las demás[^19]), y empíricamente estima el poder, a partir de hacer muchas iteraciones (repeticiones aleatorias) de esta simulación.

[^18]: No existe una única ecuación que permita hacer este cálculo.

[^19]: En particular, se necesita saber la media y desviación estándar de cada grupo y/o condición y, cuando hay factores de medidas repetidas, una matriz de correlaciones entre cada combinación de niveles.

Dadas estas complejidades, en ocasiones no es posible tener la información suficiente para hacer las simulaciones, pues se requiere de estudios previos (o pilotos) muy completos, con diseños idénticos, y que, o bien hayan reportado toda esta información, o hayan abierto libremente sus bases de datos para poder hacer estos cálculos[^20].

[^20]: Esta es una de las razones por las cuales es muy importante que en cada artículo haya una sección de descriptivos muy buena y completa, e idealmente que los datos estén disponibles para poder re-analizarlos, o hacer este tipo de análisis descriptivos necesarios para hacer un análisis de poder
...
2.  Una función para obtener el análisis de poder con base en el diseño. Para esto, hay varias opciones, incluyendo[^21]:
...
5.  `labelnames` (Opcional): las etiquetas (nombres) de los factores y sus niveles. Al igual que con las medias, estas etiquetas[^22] deben estar *concatenadas* usando la función `c`. Para definirlos, se deben poner en el siguiente orden:
...
[^23]: Por supuesto, estos datos son hipotéticos y no representan ningún estudio real, ni diferencias entre estudiantes con y sin beca, ni entre universidades públicas y privadas. Solo son usados como un ejemplo.

```{r fig7, fig.height = 4, fig.cap = "**Figura 7.** Ejemplo de la distribución de medias y sus intervalos de confianza para el diseño definido con la función `ANOVA_design`, al incluir el argumento `plot = TRUE`. Esto es muy útil para estar seguro de que las medias fueron *concatenadas* en el orden correcto."}
...
4.  `seed` (opcional): Adicionalmente, para el siguiente ejemplo usé el argumento `seed` para que las simulaciones den siempre el mismo resultado[^24].

[^24]: Dado que las bases de datos se simulan con base en las características definidas con la función `ANOVA_design`, pero **de manera aleatoria**, cada vez que corra la función obtendré un resultado ligeramente distinto, especialmente si el número de simulaciones (`nsim`) es pequeño. Al darle una semilla (`seed`), que puede ser cualquier número, los datos simulados siempre serán los mismos, garantizando que la respuesta sea la misma.
...
Este es un resultado muy interesante y completo, que incluye dos tablas; primero, una denominada `"Power and Effect sizes for ANOVA tests"` para los efectos principales e interacciones del ANOVA. Y segundo, una tabla denominada `"Power and Effect sizes for pairwise comparisons (t-tests)"` que muestra las comparaciones entre niveles de mis factores[^25] (por ejemplo, la comparación entre estudiantes becados de universidad pública y estudiantes becados de universidad privada). Ambas tablas tienen columnas que muestran:

[^25]: Ver sección [7.3 Acerca de comparaciones post-hoc](#multcomp).
...
Por ejemplo, en la primera tabla, bajo el título `"Power and Effect sizes for ANOVA tests"`, el resultado muestra que con el *n* y características propuestas, este estudio tendría un poder estadístico de 100%[^26] (o 1) para detectar el efecto principal de *Beca*, si es que este existe (es decir, una diferencia en las calificaciones del examen entre becados y no becados), cuyo tamaño del efecto se calculó en $\eta_p^2$ = 0.116942. Así mismo, un poder de 0.22 (o 22%), que es muy bajo, para detectar el efecto principal del tipo de universidad (*Universidad*: Pública, Privada), pues el efecto es sumamente pequeño, y se estimó en $\eta_p^2$ = 0.007245. Y, finalmente, un poder de 0.99 (99%) para detectar la interacción entre *Beca* y *Universidad*, pues el efecto es muy grande ($\eta_p^2$ = 0.050779).
...
En este caso, el diseño lo *guardaré* como un objeto llamado `disenoW`, y usé la opción `plot = TRUE` para asegurarme de que las medias fueron *concatenadas* en el orden correcto (**Figura 9**).

```{r fig9, fig.height = 3.4, fig.cap = "**Figura 9.** Ejemplo de la distribución de medias marginales estimadas y sus intervalos de confianza para el diseño definido con la función `ANOVA_design`, al incluir el argumento `plot = TRUE`. Esto es muy útil para estar seguro de que las medias fueron *concatenadas* en el orden correcto."}
...
Adicionalmente, tenemos que especificar la correlación entre los niveles de factores intra-sujeto (tal cual como lo hicimos para diseños de medidas repetidas), pero no para factores de medidas independientes, pues en este caso se asume siempre que la correlación es 0[^30]. De este modo, el número de coeficientes de correlación que debemos especificar (en el argumento `r` de la función `ANOVA_design`) es menor al de diseños equivalentes de medidas repetidas.

[^30]: Por esto al hacer análisis de medidas independientes no se especifica ninguna correlación.

### Ejemplo ANOVA factorial mixto (2 $\times$ 2)

Por ejemplo, si quisiera medir qué tanto un ruido insoportable (por ejemplo, el sonido de la *fresa* de un odontólogo afecta el desempeño de jugadores aficionados de ajedrez[^31], en términos de partidas ganadas sobre un total de 20 partidas jugadas, y quisiera saber si este efecto es diferente en personas según si son odontólogos o no, tendría un diseño mixto 2 $\times$ 2, pues mis factores serían:

[^31]: Es solamente un ejemplo. Jamás pensaría en torturar a jugadores de ajedrez, ni a nadie, sometiéndolo a semejante tortura solo por curiosidad "científica".
...
Como se puede ver en el código a continuación, y en la **Figura 11**, según mis datos (inventados), tanto odontólogos como no odontólogos ganan en promedio cerca de 12 partidas de 20 jugadas ($\approx$ 60%), cuando las juegan sin ruido (en morado), pero el desempeño de personas con profesiones distintas a la odontología se ve muy afectado al jugar las partidas en presencia del ruido.
...
```{r fig11, fig.height = 4,fig.cap = "**Figura 11.** Ejemplo de la distribución de medias marginales estimadas y sus intervalos de confianza para el diseño definido con la función `ANOVA_design`, al incluir el argumento `plot = TRUE`. Esto es muy útil para estar seguro de que las medias fueron *concatenadas* en el orden correcto."}
...
Puedo ver a distribución de los valores *p* para efectos principales e interacciones (**Figura 13**) con el comando:
...
```{r echo = FALSE, fig.cap = "**Figura 13.** Ejemplo de distribución (histograma) de valores *p* para efectos principales e interacciones, producto de 1000 simulaciones hechas con la función `ANOVA_power` del paquete `Superpower`. La línea roja determina el nivel de significación estadística ($\\alpha$) definido (en este caso, el típico 0.05)."}
simM$plot1
```
